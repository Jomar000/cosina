...
import { testDatabaseConfig, testKvNamespace } from './runtime.config.js'
...
/**
 * @description
 * Sends a GET request against the public app with the standard local frontend
...
export const getTestingRequest = async (
    ...
    return app.request(buildQueryPath(path, options.query), {
        method: 'GET',
        headers: {
...
/**
 * @description
 * Sends a JSON POST request against the public app with the standard local
 ...
 *
 * When using KV, this must be called within the same test process that
 * triggered the reset request.
 *
...
export const interceptPasswordResetToken = async (
     ...
     */
    const kvKeys = await testKvNamespace.list({
        prefix: 'verification:reset-password:',
    })

    for (const key of kvKeys.keys) {
        const value = await testKvNamespace.get(key.name)
        if (value) {
    ...
    const db = dbClient(testDatabaseConfig)
