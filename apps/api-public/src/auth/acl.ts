...
const buildPermissions = async (
    ...
    try {
        permissions = JSON.parse(`${await kv.getItem('cache:aclPermissions')}`)
    } catch {
        await kv.removeItem('cache:aclPermissions')
    }
    ...
    if (!permissions) {
        ...
        await kv.setItem('cache:aclPermissions', JSON.stringify(permissions), {
            ttl: 86400,
        })
    }
...
const buildRoles = async (
    ...
    try {
        roles = JSON.parse(`${await kv.getItem('cache:aclRoles')}`)
    } catch {
        await kv.removeItem('cache:aclRoles')
    }
    ...
    if (!roles) {
        ...
        await kv.setItem('cache:aclRoles', JSON.stringify(roles), {
            ttl: 86400,
        })
    }
