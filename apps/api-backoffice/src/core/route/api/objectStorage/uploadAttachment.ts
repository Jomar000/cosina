...
export const uploadAttachmentRoute = new Hono<THonoInstance>()
    ...
    .post(
        ...
        async (ctx) => {
            ...
            const objectStorageSigner = ctx.get('objectStorageSigner')
                ...
                if (uploadedObjectsHash.includes(attachment.hashSha256)) {
                    ...
                    signingPromises.push(
                        objectStorageSigner
                            .createUploadUrl({
                                bucket: uploadBucket,
                                checksumSha256Base64: hashBase64,
                                objectId: objectStorageId,
                            })
                            .then((signedUrl) => {
                                signedUrls[index].signedUrl = signedUrl
                            }),
                    )
                }
            }
            ...
            const signedUrls = await Promise.all(
                attachments.map(async (objectStorageId) => {
                    if (attachmentsToRetry.has(objectStorageId)) {
                        ...
                        const uploadBucket = isPublic ? 'public' : 'private'
                        ...
                        return {
                            ...
                            encodedHash: hashBase64,
                            signedUrl:
                                await objectStorageSigner.createUploadUrl({
                                    bucket: uploadBucket,
                                    checksumSha256Base64: hashBase64,
                                    objectId: objectStorageId,
                                }),
