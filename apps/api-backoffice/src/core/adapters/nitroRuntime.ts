...
const HYPERDRIVE_BINDING = 'HYPERIONBOFC_HD'
...
export const getAppConfig = (): TAppConfig => {
    ...
    return {
        ...
        url: {
            backend: String(service.url?.backend ?? 'http://localhost:8080'),
            frontend: String(service.url?.frontend ?? 'http://localhost:5175'),
        },
    }
}
