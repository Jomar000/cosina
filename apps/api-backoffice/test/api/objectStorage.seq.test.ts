...
const createUpload = async () => {
    const response = await app.request('/api/objectStorage/upload/create', {
        method: 'POST',
        headers: {
    ...
    const responseData = (await response.json()) as TApiResponseOk<{
        uploadId: string
    }>
...
const commitUploadAttachment = async (uploadId: string, attachmentId: string) =>
    await app.request('/api/objectStorage/upload/attachment/commit', {
        method: 'POST',
        headers: {
            origin: 'vitest-pool-worker',
            'content-type': 'application/json',
...
const commitUpload = async (uploadId: string) =>
    await app.request('/api/objectStorage/upload/commit', {
        method: 'POST',
        headers: {
            origin: 'vitest-pool-worker',
            'content-type': 'application/json',
        ...
        describe('Upload Flow', () => {
            describe('Create Upload', () => {
                it('Privileged user should be able to create an upload.', async () => {
                    ...
                    const responseData =
                        (await response.json()) as TApiResponseOk<{
                            uploadId: string
                        }>
            ...
            describe('Create Upload Attachment', () => {
                it('Should create attachment with valid input and return signed URLs.', async () => {
                    ...
                    const uploadData =
                        (await uploadResponse.json()) as TApiResponseOk<{
                            uploadId: string
                        }>
                ...
                it('Should return 409 for already uploaded (deduplicated) attachment.', async () => {
                    ...
                    const upload1Data =
                        (await upload1Response.json()) as TApiResponseOk<{
                            uploadId: string
                        }>
                    ...
                    const attach1Data =
                        (await attach1Response.json()) as TApiResponseOk<{
                            uploadId: string
                            signedUrls: {
                                id: string
                                status: number
                    ...
                    const upload2Data =
                        (await upload2Response.json()) as TApiResponseOk<{
                            uploadId: string
                        }>
            ...
            describe('Retry Upload Attachment', () => {
                it('Should regenerate signed URLs for non-uploaded attachments.', async () => {
                    ...
                    const attachData =
                        (await attachResponse.json()) as TApiResponseOk<{
                            uploadId: string
