...
- Keep modules imported into Cloudflare Worker bundles runtime-agnostic. Node-only `bootstrap.ts` and `utilities.ts` under `packages/database/src/postgres/` may use Node APIs only while they remain excluded from the database build and package exports; import them from Node/Vitest configuration code, never Worker runtime modules.
