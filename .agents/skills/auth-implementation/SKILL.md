...
4. **Sessions:** Store sessions in Cloudflare KV (`PROJECT_NAME{PUB|BOFC}_KV`) through `secondaryStorage`. Keep `session.storeSessionInDatabase` unset so Better Auth does not duplicate sessions in Postgres, and clamp KV TTL values below 60 seconds to 60 seconds.
