...
> **Note for agents and developers:** For Claude Code, these rules are **hard-enforced** by `.claude/settings.json` at the tool level — this section is a human-readable mirror of those settings. If you tighten or change `settings.json`, update this section to match. For Gemini, this section is the **only enforcement mechanism** — treat it as authoritative.
